...
export default function WalletConnect() {
  ...
  // FIX: Prevent background scrolling when any modal is open - REMOVE THIS
  // useEffect(() => {
        ...
        {showDisconnectModal &&
          ...
          createPortal(
            <div
              ...
              className="fixed inset-0 bg-[#0E121A]/75 flex items-center justify-center z-[100] p-2 sm:p-4"
            >
              <div
                className="bg-[#0E121A] border-gradient-modal rounded-lg shadow-xl w-full max-w-sm sm:max-w-md max-h-[95vh] overflow-y-auto relative mx-2"
                onClick={(e) => e.stopPropagation()}
              >
                <button
                  onClick={() => setShowDisconnectModal(false)}
                  className="absolute top-2 sm:top-4 right-2 sm:right-4 w-7 h-7 sm:w-8 sm:h-8 bg-[#434672] hover:bg-[#755A5A] text-[#E2E2E2] rounded-full flex items-center justify-center transition-colors cursor-pointer z-10 m-2"
                >
                  <svg
    ...
    <div className="relative">
      ...
      {showConnectModal &&
        ...
        createPortal(
          ...
          >
            <div
              className="bg-[#0E121A] rounded-lg shadow-xl w-full max-w-sm sm:max-w-md max-h-[95vh] overflow-y-auto relative mx-2 p-3 py-6 grid gap-5 border border-[#232542]"
              onClick={(e) => e.stopPropagation()}
            >
              <h1 className="font-anton text-2xl text-white">Connect wallet</h1>
                    ...
                    >
                      <div
                        className={`flex justify-between items-center gap-3 ${
                          connector.id === "braavos"
                            ? "bg-gradient-to-r from-[#0B4FB0] to-[#05214A]"
